...
- **Tema visual redesenhado: paleta neutra, sem acentos neon** — a interface trocou o fundo azul-marinho escuro com acentos neon (azul `#3a7bd5`, verde `#22c55e`, ciano `#00d4ff`) por uma paleta neutra em cinza escuro, no espirito do Claude Code — cor reservada para poucos sinais funcionais (sucesso, erro, modo ativo), sem saturacao "cyberpunk". Cores de syntax highlighting dentro de blocos de codigo renderizados nao foram alteradas (permanecem como em qualquer tema escuro de editor). Validado visualmente via Playwright headless carregando o webview real e simulando a timeline completa (status, cards de ferramenta, modos AUTO/HYBRID, painel de configuracoes) antes e depois da mudanca

## 0.18.3

- **Correcao: modelo local negava ter acesso a ferramentas ("como sou uma IA, nao tenho acesso ao terminal do seu computador")** — reproduzido com Qwen2.5-Coder-7B via Apple MLX. Testado isoladamente contra o `mlx_lm.server` real: com contexto limpo o modelo reconhece as ferramentas corretamente ("Sim, tenho acesso a todas as ferramentas do plugin"); a negacao so aparece depois de uma rodada anterior confusa na mesma conversa — e, uma vez dita, se autorreforca: como a resposta e salva no historico e reinjetada como contexto, o modelo passa a imitar a propria negacao em toda pergunta seguinte, mesmo quando o usuario insiste que ele tem acesso (mesma classe de bug do `[CONTINUE_BUTTON]` da 0.18.0, com conteudo diferente). Novo guard `checkCapabilityDenial` (`execution-guard.ts`) detecta essa negacao — em qualquer modo, nao so AUTO — e injeta uma correcao lembrando o modelo do schema de ferramentas real da mensagem. Verificado de ponta a ponta contra o `mlx_lm.server` real: apos o nudge, o modelo chama `list_directory` corretamente na proxima resposta

## 0.18.2

- **Correcao: modelo descrevia a acao em vez de executa-la e o agente aceitava como resposta final** — em modo AUTO, o mecanismo que detecta "modelo narrou em vez de agir" (`vou fazer X`) so cobria narracao em 1a pessoa. Reproduzido com uma resposta real: apos chamar uma ferramenta de observacao no 1o passo, o modelo caiu em modo instrucional na rodada seguinte — "Chame `run_command` para instalar as dependencias." — em vez de chamar a ferramenta de fato. Como (1) o guard `modelIsPlanning` so olha se ALGUMA ferramenta ja rodou na rodada inteira (nao no passo atual) e (2) a frase e imperativa, nao "vou X", nenhum dos dois detectores disparou, e o agente devolveu a narracao como se a tarefa estivesse concluida. Adicionados padroes para o modo imperativo/instrucional ("chame", "execute a ferramenta", "rode o comando", "call the tool") e para "vou comecar", que tambem faltava. A logica de deteccao foi extraida para `src/agent/pending-action.ts`, isolada de dependencias do VS Code, para poder ser testada diretamente
...
- **NOVO: indexacao incremental (evita contexto stale no RAG)** — indexar uma vez so não bastava: depois de editar um arquivo ja indexado, o Qdrant continuava servindo o conteudo ANTIGO como contexto, o que e uma fonte direta de alucinacao (o modelo "lembra" de codigo que nao existe mais). Agora, com RAG habilitado + backend Qdrant, o plugin reindexa automaticamente um arquivo ao salvar (`onDidSaveTextDocument`) e remove os pontos de um arquivo deletado/renomeado (`onDidDeleteFiles`/`onDidRenameFiles`) — silencioso, sem notificacoes a cada save, so uma mensagem transiente na status bar quando algo muda de fato. Cada reindex de arquivo primeiro apaga os pontos antigos daquele path antes de inserir os novos, entao um arquivo que encolheu nunca deixa "sobras" de chunks da versao mais longa anterior. Verificado: editar um arquivo indexado e re-consultar o RAG nao retorna mais o texto da versao antiga
- **Correcao: embedding degenerado (NaN) derrubava a indexacao inteira** — descoberta ao indexar este proprio repositorio: um chunk quase todo feito de regex/pontuacao fez o modelo de embedding local devolver um vetor com `NaN`, e como isso nao e JSON valido, a chamada falhava com excecao nao tratada e abortava o restante da indexacao (arquivos inteiros ficavam de fora so por causa de 1 chunk ruim). Agora todo vetor e validado (finito, sem NaN/Infinity) antes de ser usado; um chunk invalido e contado e pulado, o resto do arquivo e do workspace continua normalmente
- **Correcao critica (Apple MLX): o plugin podia mandar o modelo errado pro servidor** — o `mlx_lm.server` responde `/v1/models` escaneando **todo o cache do Hugging Face** do usuario (nao so o modelo carregado via `--model`); qualquer outro modelo mlx no mesmo cache (ex: um modelo de embedding baixado para RAG) podia aparecer ANTES do modelo em uso. O plugin confiava cegamente no "primeiro" da lista, entao digitar o id certo no campo Modelo nao garantia nada — o servidor recebia outro id e respondia 404 ("Model type X not supported"). Agora (`resolveModelId`) um id digitado explicitamente nunca e substituido: so e usado para completar um prefixo `org/` que falte, e so cai para "o primeiro da lista" quando o campo Modelo esta vazio
- **Correcao: clique acidental no botao Stop cancelava a tarefa sem o usuario perceber** — o botao Stop tinha `display:none` e so entrava no layout (empurrando o botao Enviar/Injetar) no exato instante em que o agente comecava a rodar — o momento mais provavel do usuario clicar ali de novo. Um clique destinado ao Enviar podia cair em cima do Stop recem-aparecido, cancelando a execucao sem nenhuma acao consciente. Trocado para `visibility:hidden`: o botao reserva o espaco o tempo todo, entao o layout nunca pula
- **Correcao critica: sentinel de UI vazando pro modelo causava loop infinito de repeticao** — `[CONTINUE_BUTTON]` e um marcador interno (o webview o usa so pra desenhar o botao "Continuar" e sempre o esconde do usuario) sem nenhum significado conversacional, mas o texto CRU da resposta — sentinel incluso — era salvo no historico da sessao. Como o historico e reinjetado como contexto nas proximas rodadas, o modelo passava a ver seu proprio turno anterior terminando em `[CONTINUE_BUTTON]` e, principalmente em modelos locais pequenos, comecava a imitar esse padrao: repetia a mesma frase + `[CONTINUE_BUTTON]` dezenas de vezes ate estourar o limite de passos. Agora o sentinel e removido antes de qualquer coisa ir para o historico (`extension.ts`), e removido de novo na leitura (`buildMessagesFromHistory`/`buildHistorySummary`) para saneiar sessoes que ja tinham historico contaminado salvo em disco — nao precisa limpar nada manualmente
