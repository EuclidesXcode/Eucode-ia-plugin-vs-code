...
function createThrottledStatusNotifier(post, delayMs = 150) {
    let lastSent = 0;
    let pending = null;
    let timer = null;
    const send = (text) => {
...
class EucodeViewProvider {
    ...
    resolveWebviewView(webviewView, _ctx, _token) {
        ...
        const notify = createThrottledStatusNotifier((text) => {
        ...
        this._context.subscriptions.push(vscode.window.onDidChangeActiveTextEditor(() => debouncedSendOpenFiles()), vscode.window.tabGroups.onDidChangeTabs(() => debouncedSendOpenFiles()));
        webviewView.webview.onDidReceiveMessage(async (message) => {
            ...
            if (message.image?.base64) {
                ...
                response = await (0, loop_1.runAgentLoop)(message.text, ctx.contextBlock, defaultCwd, endpoint, authHeaders, this._sessionHistory, notify, notifyCommandStart, notifyCommandOutput, makeConfirmWrite(), makeConfirmCommand(), activeModel, !!message.autoMode, this._abortController.signal, (handler) => { this._injectMessage = handler; });
                notify.flush();
                this._abortController = null;
                this._injectMessage = null;
