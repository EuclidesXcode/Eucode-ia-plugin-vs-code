...
import { getEucodePath, EUCODE_MEMORY_SUBDIR } from './workspace-init';

export interface ProjectStack {
    languages: string[];           // ['typescript', 'python']
    frameworks: string[];          // ['nextjs', 'react']
    packageManager?: string;       // 'npm' | 'yarn' | 'pnpm' | 'pip' | 'cargo' | ...
    detectedAt: number;
}

export interface MemoryDecision {
    text: string;                  // "vamos usar Zustand, nao Redux"
    addedAt: number;
    source: 'user' | 'agent';      // who recorded it
}
...
export function getSessionMemoryPath(sessionId: string): string | null {
    return getEucodePath(EUCODE_MEMORY_SUBDIR, `session_${sessionId}.json`);
}
...
export function loadSessionMemory(sessionId: string): SessionMemory {
    ...
    if (!fp || !fs.existsSync(fp)) { return emptyMemory(sessionId); }
    try {
...
function writeMemory(memory: SessionMemory): void {
...
export function rememberApprovedCommand(sessionId: string, command: string): void {
...
export function rememberDecision(sessionId: string, text: string, source: 'user' | 'agent'): { ok: boolean; reason?: string } {
    const trimmed = text.trim();
    if (!trimmed) { return { ok: false, reason: 'empty' }; }
    if (trimmed.length > 500) { return { ok: false, reason: 'too_long' }; }

    const mem = loadSessionMemory(sessionId);
    if (mem.decisions.some(d => d.text === trimmed)) { return { ok: false, reason: 'duplicate' }; }
...
export function detectAndRememberStack(sessionId: string): void {
    ...
    const has = (rel: string) => { try { return fs.existsSync(path.join(root, rel)); } catch { return false; } };
    ...
    if (has('requirements.txt') || has('pyproject.toml') || has('Pipfile')) {
        ...
        if (has('Pipfile')) { packageManager = packageManager || 'pipenv'; }
        else if (has('pyproject.toml')) { packageManager = packageManager || 'poetry'; }
        ...
        try {
            ...
            if (/\bdjango\b/i.test(reqRaw)) { frameworks.add('django'); }
            if (/\bflask\b/i.test(reqRaw)) { frameworks.add('flask'); }
            if (/\bfastapi\b/i.test(reqRaw)) { frameworks.add('fastapi'); }
        } catch { /* ignore */ }
    }
    ...
    if (has('pubspec.yaml')) {
        ...
        try {
            ...
            if (/\bflutter:/i.test(pubRaw)) { frameworks.add('flutter'); }
        } catch { /* ignore */ }
    }
...
export function buildMemorySummary(sessionId: string): string {
    const mem = loadSessionMemory(sessionId);
    const lines: string[] = [];
    ...
    if (mem.stack) {
        ...
        if (packageManager) { parts.push(`pkg=${packageManager}`); }
        if (parts.length > 0) { lines.push(`Project stack: ${parts.join(', ')}`); }
    }
...
export function dumpMemoryAsJson(sessionId: string): string {
