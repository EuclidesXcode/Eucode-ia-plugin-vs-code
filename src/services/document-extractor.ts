...
export const EXTRACTABLE_EXTS = new Set(['.xlsx', '.xlsm', '.docx', '.drawio', '.pptx']);

export function isExtractable(filePath: string): boolean {
    return EXTRACTABLE_EXTS.has(path.extname(filePath).toLowerCase());
}

// ── Leitor de ZIP via Central Directory ────────────────────────────────
// Robusto a data descriptors (compSize=0 no local header) — le tamanhos e
// offsets do Central Directory no fim do arquivo. So infla o que for pedido.
interface ZipEntry { name: string; method: number; localOffset: number; compSize: number; }
...
function inflateEntry(buf: Buffer, entry: ZipEntry): string | null {
...
function readZipMember(buf: Buffer, entries: ZipEntry[], name: string): string | null {
...
function extractDocx(buf: Buffer, entries: ZipEntry[]): string {
    const doc = readZipMember(buf, entries, 'word/document.xml');
    if (!doc) { return '[Nao foi possivel ler word/document.xml]'; }
...
function extractXlsx(buf: Buffer, entries: ZipEntry[]): { text: string; sheetCount: number } {
        ...
        for (const rowMatch of xml.matchAll(/<row[^>]*>([\s\S]*?)<\/row>/g)) {
            ...
            for (const cellMatch of rowMatch[1].matchAll(/<c\b([^>]*)>([\s\S]*?)<\/c>/g)) {
                ...
                if (type === 's') {
                    ...
                    if (v) { cells.push(strings[parseInt(v[1], 10)] ?? ''); }
...
function extractPptx(buf: Buffer, entries: ZipEntry[]): string {
...
function extractDrawio(raw: string): string {
...
export interface ExtractOptions {
    /** Teto de caracteres do texto retornado. Derivado da janela de contexto
     *  pelo chamador. Se o texto extraido passar disso, retorna resumo. */
    maxChars: number;
}
 ...
 */
export function extractDocument(filePath: string, opts: ExtractOptions): string {
