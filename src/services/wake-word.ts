...
// Continuous wake-word listener for the JARVIS mode, using only the existing
// ffmpeg + Whisper stack (no extra dependency).
//
// How it works:
//   1. Listen loop: records short ~3s windows, transcribes each via Whisper,
//      and scans the text for the wake word ("eucode").
//   2. On detection: records the actual command with ffmpeg's silencedetect
//      filter — recording stops automatically after ~5s of silence (or a hard
//      cap). The command audio is transcribed and handed to onCommand().
//   3. Loops back to listening.
//
// All recording happens in the extension host (Node), so macOS grants the mic
// permission to VSCode itself (the webview cannot use getUserMedia).

export type WakeWordLog = (msg: string) => void;

// 'acknowledging' = wake word detectada, falando "Estou ouvindo" antes de gravar.
export type WakeWordState = 'listening' | 'acknowledging' | 'recording' | 'transcribing' | 'idle';

export interface WakeWordConfig {
    ffmpegPath: string;
    deviceIndex: string;                 // avfoundation audio device index ('' = default)
    wakeWord: string;                    // e.g. "eucode"
    silenceSeconds: number;              // pause length that ends the command (default 5)
    maxCommandSeconds: number;           // hard cap on command length (default 30)
    listenWindowSeconds: number;         // wake-word probe window length (default 3)
    transcribe: (wavBuffer: Buffer) => Promise<string>; // injected: forwards to Whisper
    onCommand: (text: string) => void;   // fired with the transcribed command
...
export class WakeWordListener {
    ...
    updateConfig(partial: Partial<WakeWordConfig>): void {
    ...
    stop(): void {
        ...
        this.proc = null;
        if (p) {
            try { p.stdin.write('q'); } catch { /* noop */ }
            try { p.kill('SIGTERM'); } catch { /* noop */ }
        }
        this.log('wake-word listener parado');
    }

    private log(msg: string): void { this.cfg.log?.(`[WakeWord] ${msg}`); }
    private setState(s: WakeWordState): void { this.cfg.onState?.(s); }

    private deviceArg(): string {
    ...
    private async loop(): Promise<void> {
        while (this.running) {
            try {
                ...
                if (this.paused) { await delay(200); continue; }
                this.setState('listening');
                const probe = await this.recordWindow(this.cfg.listenWindowSeconds);
                if (!this.running) { break; }
                if (!probe || probe.length < 2048) { continue; }
    ...
    private containsWakeWord(text: string): boolean {
        ...
        if (w === 'eucode') {
            ...
            if (glued.some(v => t.includes(v))) { return true; }
        }
    ...
    private recordWindow(seconds: number): Promise<Buffer | null> {
    ...
    private async captureCommand(): Promise<void> {
        ...
        if (!text) { this.log('transcricao vazia'); return; }
        ...
        if (cleaned) { this.cfg.onCommand(cleaned); }
    }
    ...
    private runFfmpeg(args: string[], outFile: string, stopOnSilence = false): Promise<Buffer | null> {
        return new Promise((resolve) => {
            let settled = false;
            const proc = spawn(this.cfg.ffmpegPath, args);
            this.proc = proc;
...
export function classifyYesNo(text: string): 'yes' | 'no' | 'unclear' {
    ...
    if (hasNo && /\bn[aã]o\b/.test(t)) { return 'no'; } // "nao" explicito ganha
    if (hasYes && !hasNo) { return 'yes'; }
    if (hasYes && hasNo) { return 'unclear'; } // conflito → re-escuta
    return 'unclear';
}
